# Onboarding Doc Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** spring-init 템플릿 레포에 부트스트래퍼 + 신규 합류 개발자 둘 다 커버하는 단일 통합 온보딩 문서(`docs/ONBOARDING.md`) 추가, README 라우팅 허브화, GitHub Wiki 자동 미러 워크플로 구축.

**Architecture:** Source of truth는 `docs/ONBOARDING.md` (5 Parts: A 부트스트랩 / B 개발자 셋업 / C 컨벤션 / D 배포 / E 트러블슈팅). main 브랜치 push 시 워크플로가 자동으로 GitHub Wiki Home 페이지로 동기화. README는 슬림화하여 5 Part로 라우팅하는 허브.

**Tech Stack:** Markdown, GitHub Actions, GitHub Wiki.

**관련 문서:** `docs/superpowers/specs/2026-04-29-onboarding-doc-design.md`

**의존성:** PR #2(`feat/secrets-manager-cicd`)가 머지된 이후의 최종 상태를 가정하여 작성. PR #2 내용(SM 시크릿 4개 분리, DefaultCredentialsProvider, AWS_PROFILE 셋업)이 문서에 그대로 반영됨.
...
| 파일 | 변경 유형 | 책임 |
|---|---|---|
| `docs/ONBOARDING.md` | 신규 | 통합 온보딩 문서 (Part A~E) |
| `README.md` | 수정 | 슬림화 — 라우팅 허브 |
| `.github/workflows/sync_wiki.yml` | 신규 | docs/ONBOARDING.md → Wiki Home 자동 동기화 |

작업 순서: Task 1~6은 ONBOARDING.md를 섹션별로 누적. Task 7~8은 독립적이라 순서 무관. Task 9는 마지막 검증.

---

## Task 1: ONBOARDING.md 골격 + Section 0/1 작성

**Files:**
- Create: `docs/ONBOARDING.md`

- [ ] **Step 1: 파일 생성 및 초기 내용 작성**
...
- [ ] **Step 2: 마크다운 syntax 검증**

Run:
```bash
python3 -c "
content = open('docs/ONBOARDING.md').read()
...
git commit -m "docs: add ONBOARDING.md skeleton with project overview"
```

---

## Task 2: Part A — 새 프로젝트 부트스트랩

**Files:**
- Modify: `docs/ONBOARDING.md` (append)

- [ ] **Step 1: 파일 끝에 Part A 추가**

`docs/ONBOARDING.md` 끝에 다음 내용 추가:
...
Run:
```bash
grep -c "^## " docs/ONBOARDING.md
```
Expected: `4` (Section 0, 1, Part A, 그 외)

Run:
```bash
grep -c "^### A\." docs/ONBOARDING.md
```
Expected: `7` (A.1 ~ A.7)
...
```bash
git add docs/ONBOARDING.md
git commit -m "docs: add Part A — bootstrap new project guide"
```

---

## Task 3: Part B — 개발자 합류 셋업

**Files:**
- Modify: `docs/ONBOARDING.md` (append)

- [ ] **Step 1: 파일 끝에 Part B 추가**
...
```bash
git add docs/ONBOARDING.md
git commit -m "docs: add Part B — developer onboarding setup guide"
```

---

## Task 4: Part C — 코드 컨벤션 & 아키텍처

**Files:**
- Modify: `docs/ONBOARDING.md` (append)

- [ ] **Step 1: 파일 끝에 Part C 추가**
...
```bash
git add docs/ONBOARDING.md
git commit -m "docs: add Part C — code conventions and architecture"
```
...
```bash
git add docs/ONBOARDING.md
git commit -m "docs: add Part D — deployment and operations"
```
...
```bash
git add docs/ONBOARDING.md
git commit -m "docs: add Part E — troubleshooting guide"
```

---

## Task 7: README.md 슬림화 (라우팅 허브)
...
onboarding = open('docs/ONBOARDING.md').read()

# README의 ONBOARDING.md anchor 링크 모두 추출
links = re.findall(r'docs/ONBOARDING\.md#([^)]+)', readme)
print(f"README has {len(links)} anchor links to ONBOARDING.md")

# ONBOARDING.md의 헤딩들에서 GitHub anchor 생성
headings = re.findall(r'^(#{1,4}) (.+)$', onboarding, re.MULTILINE)
def to_anchor(h):
    s = h.lower().replace(' ', '-')
    s = re.sub(r'[^\w가-힣\-]', '', s)
    return s
anchors = {to_anchor(h[1]) for h in headings}

missing = [link for link in links if link not in anchors]
if missing:
    print(f"MISSING anchors: {missing}")
    exit(1)
print("All README anchors resolve in ONBOARDING.md")
EOF
```
Expected: `All README anchors resolve in ONBOARDING.md`

- [ ] **Step 3: Commit**

```bash
git add README.md
git commit -m "docs: slim README to routing hub for ONBOARDING.md"
```

---

## Task 8: GitHub Wiki 자동 동기화 워크플로

**Files:**
- Create: `.github/workflows/sync_wiki.yml`
      ...
      - name: Commit & push if changed
        ...
        run: |
          ...
          git diff --cached --quiet || (
            git commit -m "sync: ONBOARDING.md → Home"
            git push
          )
```
...
Run:
```bash
python3 -c "import yaml; yaml.safe_load(open('.github/workflows/sync_wiki.yml'))" && echo "YAML OK"
```
Expected: `YAML OK`

- [ ] **Step 3: Wiki 활성화 안내 ONBOARDING.md에 추가**

`docs/ONBOARDING.md`의 Part A.6 끝에 다음 한 줄 추가 ("(선택)" 섹션 직전):

```bash
python3 << 'EOF'
content = open('docs/ONBOARDING.md').read()
needle = "### A.7 (선택) 도메인 + HTTPS 셋업"
replacement = """> 📚 **Wiki 활성화 (1회)**: 레포 Settings → General → Features → Wiki 체크박스 활성화. 그 후 Wiki 탭에서 'Create the first page'를 1회 클릭 (내용은 임시로 둠). 이후 main 브랜치에 docs/ONBOARDING.md가 변경되면 `Sync Wiki` 워크플로가 자동으로 Wiki Home을 갱신한다.

### A.7 (선택) 도메인 + HTTPS 셋업"""
new = content.replace(needle, replacement)
assert new != content, "Replacement failed"
open('docs/ONBOARDING.md', 'w').write(new)
print("Wiki activation note added")
EOF
```
Expected: `Wiki activation note added`

- [ ] **Step 4: Commit**

```bash
git add .github/workflows/sync_wiki.yml docs/ONBOARDING.md
git commit -m "ci: auto-sync ONBOARDING.md to GitHub Wiki Home on main push"
```
...
- [ ] **Step 1: 전체 ONBOARDING.md 구조 점검**

Run:
```bash
grep -c "^## Part [A-E]\." docs/ONBOARDING.md
```
Expected: `5` (Part A, B, C, D, E)
...
- [ ] **Step 2: 모든 internal anchor 정합성 점검**
...
internal_links = re.findall(r'\(#([^)]+)\)', content) + re.findall(r'docs/ONBOARDING\.md#([^)]+)', readme)
...
anchors = {to_anchor(h[1]) for h in headings}
missing = sorted(set(link for link in internal_links if link not in anchors))
if missing:
    print(f"MISSING: {missing}")
    exit(1)
print(f"All {len(internal_links)} internal/README links resolve")
EOF
```
Expected: `All N internal/README links resolve`

- [ ] **Step 3: 변경 통계 확인**

Run:
```bash
git diff --stat main..HEAD -- README.md docs/ONBOARDING.md .github/workflows/sync_wiki.yml
```
Expected: 3개 파일, ONBOARDING.md ~600줄 추가, README.md 변경, sync_wiki.yml 추가.

Run:
```bash
git log --oneline main..HEAD
```
Expected: 8개 commit (Task 1~8).

- [ ] **Step 4: 워크플로 트리거 검증 (선택)**

PR 머지 후 main에 push되면 `Sync Wiki` 워크플로가 자동 실행. Actions 탭에서 ✅ 확인.

(Wiki가 처음 생성되어 있어야 함 — Task 8 Step 3의 안내대로 1회 활성화)
...
| Spec 섹션 | 구현 Task |
|---|---|
| §3.1 파일 구조 (3개 파일) | Task 1, 7, 8 |
| §3.1 Wiki 동기화 워크플로 | Task 8 |
| §4 ONBOARDING.md 목차 - 0/1 | Task 1 |
| §4 Part A | Task 2 |
| §4 Part B | Task 3 |
| §4 Part C | Task 4 |
| §4 Part D | Task 5 |
...
**Placeholder 점검:** 모든 단계에 실제 markdown 콘텐츠 또는 명령. "TODO/TBD" 없음. 단, `<projectName>`, `<프로젝트명>` 등은 ONBOARDING 본문에서 의도적으로 placeholder로 남겨둠 (독자가 자기 프로젝트명으로 치환).

**일관성 점검:** Part A~E 헤딩 규약(`### X.Y`) 일관. README anchor와 ONBOARDING heading anchor 일치 (Task 7 Step 2 + Task 9 Step 2 자동 검증).
